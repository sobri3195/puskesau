...
const Dashboard: React.FC = () => {
  ...
  const { notifications: notificationData, setNotifications: setNotificationData } = useOpsData();
    ...
    const intervalId = setInterval(() => {
      ...
      setTimeout(() => {
        setStatCards((prevCards) => prevCards.map((card) => {
          ...
          if (card.title === 'Total Pasien Aktif') change = Math.floor(Math.random() * 10) - 5;
        ...
        setLogisticsData((prevData) => ({
          ...
          data: prevData.data.map((series) => ({
            ...series,
            values: series.values.map((val, index) => index === series.values.length - 1 ? Math.max(0, Math.min(100, val + Math.floor(Math.random() * 10) - 5)) : val),
          })),
        }));
        ...
        setLowStockData((prevData) => {
          const newData = prevData.map((item) => {
            ...
            const stockChange = Math.random() < 0.05
              ? Math.floor(Math.random() * 20) + 10
              : -(Math.floor(Math.random() * (item.status === StockStatus.Kritis ? 4 : 3)) + (item.status === StockStatus.Kritis ? 1 : 0));
